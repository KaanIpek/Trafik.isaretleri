...
def sayItem():
    ...
    while True:
        if speak == True:
            var = gTTS(text=ses, lang='tr')
            var.save('sound'+str(k)+'.mp3')
            playsound('sound'+str(k)+'.mp3')
            k+=1
            speak = False
...
SIGNS = ["Bulunamadi","Dur","Sola dön","Sağa dön","Sola dönülmez","Sağa dönülmez","Girişi olmayan yol","Hız limiti 40","Hız limiti 30","Sağa mecburi dönüş","Diğer"]
print(SIGNS)
# Önceden bulunan resimleri silmek için
def clean_images():
...
def main(args):
    ...
    while True:
        ...
        if sign_type > 0 and (not current_sign or sign_type != current_sign):
            ...
            cv2.putText(frame, str(round(fpsFilter, 1)) + '  fps  ' + ses + '   ' + str(round(confidence, 2)), (0, 30),
                        font, 1, (0, 0, 255), 2)
            ...
            if sign_type > 0:
                ...
                fpsFilter = .95 * fpsFilter + .05 * fps
                cv2.putText(frame, str(round(fpsFilter, 1)) + '  fps  ' + ses + '   ' + str(round(confidence, 2)),
                            (0, 30), font, 1, (0, 0, 255), 2)
                cv2.imshow('myCam', frame)
                cv2.moveWindow('myCam', 1000, 300)
                if cv2.waitKey(1) == ord('q'):
                    break
